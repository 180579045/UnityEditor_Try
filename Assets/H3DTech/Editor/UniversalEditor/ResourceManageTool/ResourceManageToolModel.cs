...
public class ResourceManageToolModel  
{
    ...
    {  
        ...
        if (tryLoadCache)
        {
            //从Cache文件中读取所有信息到对应的Dictionary
            if (U3DAssetDB.GetInstance().TryLoad())
            {
                //从Dictionary中读取所有资源（这个所有资源没有了索引，存成List有什么用？）
                //和Dictionary中读取的所有无用资源
                _UpdateAllAssetList();

                //异步回调ManagerToolControler中的处理
    ...
    private void _NotifyAssetDatabaseUpdate()
    {
        ...
        {
            Debug.Log("_NotifyAssetDatabaseUpdate中有没有赋值给委托？");
    ...
    private void _NotifyAssetDatabaseStateChange()
    {
        ...
        {
            Debug.Log("_NotifyAssetDatabaseStateChange中有没有赋值给委托？");
