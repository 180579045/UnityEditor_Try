...
public class ResourceRefTool 
{
    ...
    //add by liteng for Tool Repot

    static private string m_helpURL = "http://192.168.2.121:8090/pages/viewpage.action?pageId=6619510&moved=true";

    [MenuItem("H3D/资源管理/资源检查工具")]
    static void Init()
    {
        //一个单例模式，UI框架只允许同时实例化一个同名UI;
        EditorRoot root = EditorManager.GetInstance().FindEditor("资源检查工具");
        if (root == null)
        {
    ...
    static void OnFilterComboSelectChanged( EditorControl c , object sel )
    {
        ResourceManageToolModel.GetInstance().SetFilter((int)sel);
    }
