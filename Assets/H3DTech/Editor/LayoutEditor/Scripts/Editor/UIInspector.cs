...
public class UIInspector
{
    ...
    public static void DrawUIProp(List<UIElement> elementList, Rect rect)
    {
        if (elementList.Count > 1 || elementList.Count == 0)
        {
            if (element != null)
            {
                element.SyncPrefabUI((int)EUIOpFlag.UOF_Attribute | (int)EUIOpFlag.UOF_Shape);
    ...
    private static void DrawUILabelProp()
    {
        ...
        ComponentSelector.Draw<Font>(mLabel.trueTypeFont, OnSelectFont, true);

        GUILayout.BeginHorizontal();
        int nfontsize = EditorGUILayout.IntField("字体大小", mLabel.fontSize, GUILayout.Width(150f));
        if (nfontsize != mLabel.fontSize)
        {
            RegisterUndo();
            mLabel.fontSize = nfontsize;
        }

        FontStyle fStyle = (FontStyle)EditorGUILayout.EnumPopup("", mLabel.fontStyle, GUILayout.Width(150f));
        if (fStyle != mLabel.fontStyle)
        {
            RegisterUndo();
            mLabel.fontStyle = fStyle;
        }
        GUILayout.EndHorizontal();
        ...
        //if (mLabel.font != null)
        {
            ...
            */
            ...
            { 
                RegisterUndo(); 
                mLabel.maxLineCount = count;
            }

            Color fontColor = EditorGUILayout.ColorField("颜色", mLabel.color);
            if (mLabel.color != fontColor)
            {
                RegisterUndo();
                mLabel.color = fontColor;
            }
    ...
    static void OnSelectFont(UnityEngine.Object obj)
    {
        if (mLabel != null)
        {
            //RegisterUndo();
    ...
    static void UISpriteDrawExtraProperties()
    {
        ...
        {
            bool fillCenter = mSprite.centerType != UISprite.AdvancedType.Invisible;
            bool fill = EditorGUILayout.Toggle("Fill Center", fillCenter);
            ...
            if (fillCenter != fill)
            {
                ...
                if (fill != fillCenter)
                {
                    mSprite.centerType = fill ? UISprite.AdvancedType.Sliced : UISprite.AdvancedType.Invisible;
